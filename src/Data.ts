...
export class Data {
  ...
  public registerCommands(analyse: Command) {
    analyse.subcommand('.backup', '备份统计数据')
      .action(() => this.backupData());

    analyse.subcommand('.restore', '恢复统计数据')
      .action(() => this.restoreData());

    analyse.subcommand('.clear', '清理统计数据')
      ...
      .action(async ({ options }) => {
        ...
        return this.clearData(options);
      });

    analyse.subcommand('.list', '列出频道和命令')
      .action(() => this.listData());
  }
    ...
    try {
      ...
      if (usersToImport.length) {
        for (let i = 0; i < usersToImport.length; i += BATCH_SIZE) {
          await this.ctx.database.upsert('analyse_user', usersToImport.slice(i, i + BATCH_SIZE));
        }
      }
      ...
      for (const tableName of ALL_TABLES.filter(t => t !== 'analyse_user')) {
        ...
        if (recordsWithUid.length > 0) {
          for (let i = 0; i < recordsWithUid.length; i += BATCH_SIZE) {
            await this.ctx.database.upsert(tableName, recordsWithUid.slice(i, i + BATCH_SIZE));
          }
        }
      }
      return '数据恢复成功';
  ...
  /** @description 清理数据 */
  private async clearData(options) {
    try {
      if (options.all) {
      ...
      for (const tableName of tablesToClear) {
        await this.ctx.database.remove(tableName as any, query);
      }
